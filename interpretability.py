...
def convert_to_img(args, rgb_img, activation_map):
    ...
    for f in range(num_features):
        for t in range(1):#num_frames):
            ...
            activation_frame_dst[..., 3] = np.arange(len(activation_frame_dst[..., 3]))#activation_frame#np.array(activation_frame) / 255.0
            print("activation_frame_dst[0,0]:", activation_frame_dst[..., 3])
            activation_frame_dst = Image.fromarray(activation_frame_dst).resize((width, height), PIL.Image.NEAREST)
